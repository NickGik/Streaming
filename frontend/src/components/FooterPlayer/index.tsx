...
interface FooterPlayerProps {
  presenter: PlayerPresenter; // Презентер плеера
}

// Компонент FooterPlayer
const FooterPlayer: React.FC<FooterPlayerProps> = ({ presenter }) => {
  // Состояние для текущей песни (изначально null)
  const [currentSong, setCurrentSong] = useState<Song | null>(null); 

  // Состояние для флага воспроизведения (изначально значение из презентера)
  const [isPlaying, setIsPlaying] = useState(presenter.getIsPlaying());

  // Состояние для громкости (изначально значение из презентера)
  const [volume, setVolume] = useState(presenter.getVolume());

  // Состояние для текущего времени воспроизведения (изначально значение из презентера)
  const [currentTime, setCurrentTime] = useState(presenter.getCurrentTime());

  // Состояние для общей длительности песни (изначально 0)
  const [duration, setDuration] = useState(0);

  // Состояние для флага случайного воспроизведения (изначально значение из презентера)
  const [isShuffling, setIsShuffling] = useState(presenter.getIsShuffling());

  // Состояние для флага повтора (изначально значение из презентера)
  const [isRepeating, setIsRepeating] = useState(presenter.getIsRepeating());

  // useEffect для обработки событий презентера
  useEffect(() => {
    console.log("FooterPlayer - useEffect - компонент смонтирован");

    // Функция для обновления состояния текущей песни
    const updateCurrentSong = () => {
      console.log("FooterPlayer - updateCurrentSong() - обновление текущей песни");
      const song = presenter.getCurrentSong(); // Получаем текущую песню из презентера
      setCurrentSong(song); // Обновляем состояние currentSong
      setDuration(song ? song.duration / 1000 : 0); // Устанавливаем длительность песни
    };

    // Функция для обновления состояния флага воспроизведения
    const updateIsPlaying = () => {
      console.log("FooterPlayer - updateIsPlaying() - обновление состояния воспроизведения");
      setIsPlaying(presenter.getIsPlaying()); // Получаем флаг воспроизведения из презентера
    };

    // Функция для обновления состояния громкости
    const updateVolume = () => {
      console.log("FooterPlayer - updateVolume() - обновление громкости");
      setVolume(presenter.getVolume()); // Получаем громкость из презентера
    };

    // Функция для обновления состояния текущего времени воспроизведения
    const updateCurrentTime = () => {
      console.log("FooterPlayer - updateCurrentTime() - обновление текущего времени");
      const newCurrentTime = presenter.getCurrentTime();
      console.log("New Current Time:", newCurrentTime); // Добавьте эту строку
      setCurrentTime(newCurrentTime); 
    };

    // Подписываемся на события презентера
    presenter.onSongChange = updateCurrentSong;
    presenter.onPlayPauseChange = updateIsPlaying;
    presenter.onVolumeChange = updateVolume;
    presenter.onTimeUpdate = updateCurrentTime;

    // Функция очистки при размонтировании компонента
    return () => {
      console.log("FooterPlayer - useEffect - компонент размонтирован");
      // Отписываемся от событий презентера
      presenter.onSongChange = null;
      presenter.onPlayPauseChange = null;
      presenter.onVolumeChange = null;
      presenter.onTimeUpdate = null;
    };
  }, [presenter]); // Зависимость от презентера

  // Обработчик нажатия кнопки Play/Pause
  const handlePlayPause = () => {
    console.log("FooterPlayer.handlePlayPause() - нажатие кнопки Play/Pause");
    if (isPlaying) {
      presenter.pause(); // Вызываем метод pause презентера
    } else {
      presenter.play(); // Вызываем метод play презентера
    }
  };

  // Обработчик нажатия кнопки Next Track
  const handleNextTrack = () => {
    console.log("FooterPlayer.handleNextTrack() - нажатие кнопки Next Track");
    presenter.nextTrack(); // Вызываем метод nextTrack презентера
  };

  // Обработчик нажатия кнопки Previous Track
  const handlePrevTrack = () => {
    console.log("FooterPlayer.handlePrevTrack() - нажатие кнопки Previous Track");
    presenter.prevTrack(); // Вызываем метод prevTrack презентера
  };

  // Обработчик изменения громкости
  const handleVolumeChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    const newVolume = parseFloat(event.target.value);
    console.log(`FooterPlayer.handleVolumeChange() - изменение громкости: ${newVolume}`);
    presenter.setVolume(newVolume); // Вызываем метод setVolume презентера
  };
  ...
  const handleTimeChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    console.log(`FooterPlayer.handleTimeChange() - изменение времени: ${newTime}`);
    presenter.seek(newTime); // Вызываем метод seek презентера
  };

  // Обработчик переключения случайного воспроизведения
  const handleToggleShuffle = () => {
    console.log("FooterPlayer.handleToggleShuffle() - переключение случайного воспроизведения");
    presenter.toggleShuffle(); // Вызываем метод toggleShuffle презентера
    setIsShuffling(presenter.getIsShuffling()); // Обновляем состояние isShuffling
  };

  // Обработчик переключения повтора
  const handleToggleRepeat = () => {
    console.log("FooterPlayer.handleToggleRepeat() - переключение повтора текущей песни");
    presenter.toggleRepeat(); // Вызываем метод toggleRepeat презентера
    setIsRepeating(presenter.getIsRepeating()); // Обновляем состояние isRepeating
  };
  ...
  return (
    <div className="footer-player">        
      ...
      <div className="controls">
        ...
        <button onClick={handlePrevTrack}> {/* Кнопка Previous Track */}
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <button onClick={handlePlayPause}> {/* Кнопка Play/Pause */}
      ...
      <div className="volume-control">
        <svg /* ... */ /> 
        <div className="volume-background"> {/* Добавляем новый div */}
          <div 
